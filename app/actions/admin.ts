...
export async function adminGetUsers(page = 1, limit = 20, search?: string) {
  ...
  let query = supabase
    .from('profiles')
    .select(`*, subscriptions(plan, status, current_period_end, selected_charity_id, monthly_fee_gbp)`, { count: 'exact' })
    .eq('is_admin', false)
...
export async function adminUpdateUserScore(scoreId: string, newScore: number, newDate: string) {
  await requireAdmin()
  const supabase = await createServiceClient()
...
export async function adminToggleSubscription(userId: string, activate: boolean) {
  await requireAdmin()
  const supabase = await createServiceClient()
  const { error } = await supabase
    .from('subscriptions')
...
export async function adminDeleteCharity(id: string) {
  ...
  if (count && count > 0) {
    return { error: `Cannot delete: ${count} active subscriber(s) have selected this charity.` }
  }

  const { error } = await supabase.from('charities').update({ status: 'inactive' }).eq('id', id)
  if (error) return { error: error.message }
...
export async function adminVerifyWinner(winnerId: string, approved: boolean, notes?: string) {
...
export async function adminMarkWinnerPaid(winnerId: string, reference: string) {
  ...
  const { error } = await supabase.from('winners').update({
    ...
    status:            'paid',
  }).eq('id', winnerId).eq('status', 'verified')
...
export async function adminCreateNextMonthPool() {
  ...
  const totalFees = subs?.reduce((acc, s) => acc + s.monthly_fee_gbp, 0) ?? 0
  const avgCharity = subs?.reduce((acc, s) => acc + s.charity_percentage, 0) / (subs?.length || 1) ?? 10
  ...
  if (error || !pool) return { error: error?.message ?? 'Failed to create pool' }
