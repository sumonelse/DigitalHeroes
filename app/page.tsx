...
import { PrizePool, CharitySpotlight, PricingSection } from '@/components/features/home/PrizePool'
import { Navigation } from '@/components/features/home/Navigation'
import { Footer } from '@/components/features/home/Footer'

async function getHomepageData() {
  const supabase = await createClient()
  const [{ data: charities }, { data: pool }, { data: draws }] = await Promise.all([
    supabase.from('charities').select('*').eq('status', 'featured').order('featured_order').limit(3),
    supabase.from('prize_pools').select('*').order('created_at', { ascending: false }).limit(1).single(),
