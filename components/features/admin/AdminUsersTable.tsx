...
export function AdminUsersTable({
  ...
  const handleToggle = async (userId: string, currentStatus: string) => {
    ...
    startTransition(async () => {
      await adminToggleSubscription(userId, activate)
      router.refresh()
      setToggling(null)
    })
  }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5">
                {['Player', 'Country', 'Plan', 'Status', 'Joined', 'Actions'].map(h => (
              ...
              {users.map((user, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 capitalize text-white/60">{sub?.plan ?? '—'}</td>
                    <td className="px-5 py-4">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS_STYLES[status] ?? STATUS_STYLES.inactive}`}>
                        {status}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      ...
                      >
                        {toggling === user.id ? '…' : status === 'active' ? 'Deactivate' : 'Activate'}
                      </button>
                    </td>
                  </motion.tr>
                )
