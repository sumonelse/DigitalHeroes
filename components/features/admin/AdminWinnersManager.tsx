...
export function AdminWinnersManager({ winners: initialWinners }: { winners: WinnerWithRelations[] }) {
  ...
  const handleVerify = async (id: string, approved: boolean) => {
    ...
    if (result.success) {
      setWinners(prev => prev.map(w =>
        w.id === id ? { ...w, status: approved ? 'verified' : 'rejected', review_notes: notes[id] ?? null } : w
      ))
      setExpanded(null)
    }
  ...
  const handlePaid = async (id: string) => {
    if (!payRef[id]?.trim()) { alert('Please enter a payment reference.'); return }
    setLoading(prev => ({ ...prev, [id]: true }))
    const result = await adminMarkWinnerPaid(id, payRef[id])
    if (result.success) {
      setWinners(prev => prev.map(w =>
        w.id === id ? { ...w, status: 'paid', payment_status: 'paid', payment_reference: payRef[id] } : w
      ))
    }
    setLoading(prev => ({ ...prev, [id]: false }))
  }

  const totalPending  = winners.filter(w => w.status === 'pending').reduce((a, w) => a + w.prize_amount_gbp, 0)
  const totalVerified = winners.filter(w => w.status === 'verified').reduce((a, w) => a + w.prize_amount_gbp, 0)
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Approved, Unpaid', amount: totalVerified, color: 'text-[#4488ff]' },
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {(['all', 'pending', 'verified', 'paid', 'rejected'] as const).map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
        ...
        <AnimatePresence>
          {filtered.map(winner => (
            ...
            >
              ...
              >
                {/* Match type */}
                <div className="text-2xl flex-shrink-0">{MATCH_ICONS[winner.match_type] ?? '🎯'}</div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-white/40 truncate">{winner.profiles?.email}</div>
                </div>
                ...
                {/* Status */}
                <div className={`text-xs font-bold px-3 py-1.5 rounded-full border flex-shrink-0 ${STATUS_STYLES[winner.status] ?? STATUS_STYLES.pending}`}>
                  {winner.status}
                </div>
                  ...
                  >
                    <div className="border-t border-white/5 p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          <p className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-2">Winning Numbers</p>
                          <div className="flex gap-2">
                            {winner.draws?.winning_numbers?.map((n, i) => (
                              <span key={i} className="draw-ball w-10 h-10 text-base">{n}</span>
                            ))}
                          </div>
                        </div>
                        ...
                        {winner.proof_url && (
                          <div>
                            <p className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-2">Proof Submitted</p>
                            <a
                            ...
                            {winner.proof_uploaded_at && (
                              <p className="text-xs text-white/30 mt-1">
                                Uploaded {new Date(winner.proof_uploaded_at).toLocaleDateString('en-GB')}
                              </p>
                            )}
                          </div>
                        )}
                        ...
                        {winner.review_notes && (
                          <div>
                            <p className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-1">Review Notes</p>
                        ...
                        {winner.payment_reference && (
                          <div>
                            <p className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-1">Payment Reference</p>
                            <p className="text-sm text-emerald font-mono">{winner.payment_reference}</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-4">
                        {winner.status === 'pending' && (
                          <>
                            <p className="text-xs font-semibold text-white/40 uppercase tracking-widest">Review Decision</p>
                            <textarea
                            ...
                            <div className="flex gap-3">
                              ...
                              >
                                {loading[winner.id] ? 'Processing…' : '✓ Approve'}
                              </button>
                              <button
                        ...
                        {winner.status === 'verified' && (
                          <>
                            <p className="text-xs font-semibold text-white/40 uppercase tracking-widest">Mark as Paid</p>
                            <input
                              type="text"
                              value={payRef[winner.id] ?? ''}
                              onChange={e => setPayRef(prev => ({ ...prev, [winner.id]: e.target.value }))}
                            ...
                            >
                              {loading[winner.id] ? 'Marking paid…' : '💸 Mark as Paid'}
                            </button>
                          </>
                        )}
                        ...
                        {(winner.status === 'paid' || winner.status === 'rejected') && (
                          <div className={`p-4 rounded-xl text-sm ${
                            winner.status === 'paid' ? 'bg-emerald/10 text-emerald border border-emerald/20' : 'bg-rose/10 text-rose border border-rose/20'
                          }`}>
                            {winner.status === 'paid'
