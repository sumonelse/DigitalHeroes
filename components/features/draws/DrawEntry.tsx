...
export function DrawEntry({
  ...
  const handleEnter = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setEnteredEntry({
          submitted_scores: result.scores!,
          matched_count: null,
          is_winner: false,
        });
        setJustEntered(true);
        ...
        {draw ? (
              ...
              {pool && (
                <div className="grid grid-cols-3 gap-3 mb-8">
                  {[
                    {
                      label: "Jackpot",
                      val: pool.jackpot_pool_gbp,
                      color: "gold",
                      rolled: pool.jackpot_rolled_over,
                    },
                    {
                      label: "4-Match",
                      val: pool.match4_pool_gbp,
                      color: "emerald",
                    },
                    {
                      label: "3-Match",
                      val: pool.match3_pool_gbp,
                      color: "sapphire",
                    },
                  ].map((tier) => (
                    <div
                ...
                <div>
                  {enteredEntry ? (
                    <AnimatePresence>
                      ...
                      >
                        {justEntered && (
                          ...
                          >
                            <div className="w-8 h-8 rounded-full bg-emerald/20 border border-emerald/30 flex items-center justify-center text-emerald text-lg">
                              ✓
                            </div>
                            <div>
                              ...
                              <p className="text-xs text-white/50">
                                Draw runs at end of month. Good luck!
                              </p>
                            </div>
                          </motion.div>
                        )}
                        ...
                        <div>
                          ...
                          <p className="text-xs text-white/30 mt-3">
                            Your numbers are locked in. Scores are updated
                            automatically if you edit them.
                          </p>
                        </div>
                      </motion.div>
                    </AnimatePresence>
                    ...
                    <div className="space-y-5">
                      <div>
                        <p className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-4">
                          Your Numbers (from your 5 scores)
                        </p>
                        {scores.length === 5 ? (
                          <div className="flex gap-3 flex-wrap">
                            {scores.map((s, i) => (
                          ...
                          <div className="flex gap-3">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <div
                                key={i}
                                className="draw-ball border-dashed opacity-30 text-white/20"
                              >
                                ?
                              </div>
                            ))}
          ...
          <div className="glass rounded-card p-12 text-center">
            ...
            <p className="text-white/40">
              The next draw will be set up soon by the admin. Check back
              shortly.
            </p>
          </div>
        )}
                    ...
                    <div>
                      ...
                      {userEntry && (
                        <div className="flex gap-1 mt-1">
                          {userEntry.submitted_scores?.map(
                            (s: number, i: number) => (
                              <span
                                key={i}
                                className={`text-[10px] px-1.5 py-0.5 rounded font-mono font-bold ${
                                  pd.winning_numbers?.includes(s)
                        ...
                        <div className="flex gap-1">
                          {pd.winning_numbers
                            ...
                            .map((n: number, i: number) => (
                              <span
                                key={i}
                                className="w-7 h-7 rounded-full bg-gold/10 text-gold text-xs flex items-center justify-center font-bold"
                              >
                                {n}
                              </span>
                            ))}
      ...
      <div className="space-y-5">
            ...
            {[
              {
                icon: "⛳",
                step: "1. Log 5 scores",
                desc: "Enter your Stableford rounds — each score becomes a draw number.",
              },
              {
                icon: "✦",
                step: "2. Auto-entered",
                desc: "Once you have 5 scores and an active subscription, enter the draw.",
              },
              {
                icon: "🎯",
                step: "3. Monthly draw",
                desc: "At month end, 5 winning numbers are drawn — random or weighted.",
              },
              {
                icon: "🏆",
                step: "4. Win prizes",
                desc: "Match 3, 4, or all 5 numbers to claim your share of the prize pool.",
              },
            ].map((item) => (
        ...
        <div className="glass rounded-card p-6">
          ...
          <div className="space-y-3">
            {[
              ...
              >
                ...
                <span
                  className={`font-display text-lg font-bold ${tier.color}`}
                >
                  {tier.pct}
                </span>
              </div>
          ...
          <p className="text-xs text-white/20 mt-4">
            Jackpot rolls over if no 5-match winner. Prizes split equally among
